...
df = pd.read_csv("data/test_data.csv", names=["id", "property_description", "area_jhb", "listing_agency", "street_address", "occupation_date", "Bedrooms_1", "Bedrooms_2", "bathrooms", "area", "Feature_1", "Feature_2", "additional"])
df = df[df["id"] != "id"]
...
df["Extracted area from description"] = df["property_description"].str.extract("Size:\s(.*)\sm")
...
df= pd.concat([dfa, dfb, df], ignore_index=True)
...
df["Extracted Bathrooms"] = df[df["Bedrooms_1"] == "Beds"]["bathrooms"]
...
# dfbeds.loc[pd.to_numeric(dfbeds["Bedrooms_1"], errors="coerce").isnull(), "Bedrooms_1"] = np.nan
# df["Extracted Bathrooms"] = df["Extracted Bathrooms"].replace(np.nan, "0").str.replace("\s", "")
...
df = df[["id", "Extracted Bedrooms", "Extracted area", "Extracted Bathrooms", "Extracted Area_JHB", "Extracted listing agency", "Extracted Type", "Extracted Available Month"]]
print(df)
